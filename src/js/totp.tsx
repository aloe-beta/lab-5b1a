...
class Totp {
    ...
    editCard(card: totpCard | null) {
        ...
        if (card) {
            menu.name.value = card.name;
            menu.secret.value = card.secret;
            menu.interval[card.interval === 30 ? 0 : 1].checked = true;
            menu.save.classList.toggle('disabled', false);
        }
